...
add_button:connect_signal("mouse::enter", function(c) c:set_bg("#24283") end)
add_button:connect_signal("mouse::leave", function(c) c:set_bg("#24283b") end)
...
local function worker(user_args)
  ...
  function update_widget(stdout)
    ...
    local first_row = wibox.widget {
      {
        { widget = wibox.widget.textbox },
        {
          markup = '<span size="large" font_weight="bold" color="#ffcfc2">Todo  List</span>',
          align = 'center',
    ...
    for i, todo_item in ipairs(result.todo_items) do
      ...
      row:connect_signal("mouse::leave", function(c) c:set_bg("#24283b") end)
    ...
    gears.table.join(
      awful.button({}, 1, function()
        if popup.visible then
          ...
          todo_widget.widget:set_bg("#24283b")
